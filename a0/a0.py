...
def get_users(twitter, screen_names):
    ...
    ###TODO
    user = []
    request = robust_request(twitter, 'users/lookup',
                             {'screen_name': screen_names, 'count': 100})
...
def followed_by_hillary_and_donald(users, twitter):
    ...
    for friend in commonfriends:
        request = robust_request(twitter, 'users/lookup', {'user_id': friend})
        user = [item for item in request]
        users.append(user[0]['screen_name'])
    return users
...
def create_graph(users, friend_counts):
    ...
    ###TODO
    graph = nx.Graph()
    friends_sev = friend_counts.copy()
    friends_sev = {ids: friends for ids,
                   friends in friends_sev.items() if friends > 1}
    for i in range(0, len(users)):
        graph.add_node(users[i]['screen_name'])
        for id in friends_sev:
...
def draw_network(graph, users, filename):
    ...
    i = 0
    for user in users:
        labels[i] = user['screen_name']
        p[i] = pos[user['screen_name']]
        i = i + 1
    nx.draw(graph, pos=pos, with_labels=False, node_size=50, alpha=0.5)
    nx.draw_networkx_labels(graph, pos=p, labels=labels)
...
def main():
    ...
    print('User followed by Hillary and Donald: %s' %
          str(followed_by_hillary_and_donald(users, twitter)))
    graph = create_graph(users, friend_counts)
    print('graph has %s nodes and %s edges' %

    ...
    while (len(q) > 0):  # while more to visit
      ...
      if n not in seen:
        ...
        for nn in graph.neighbors(n):
          if node2distances[n]+1 > max_depth:
              break
          if nn not in seen:
            q.append(nn)
            if node2distances[n]+1 > max_depth:
            ...
            if node2distances[n] < node2distances[nn]:
...
def partition_girvan_newman(graph, max_depth):
    ...
    edge2betweenness = sorted(edge2betweenness.items(), key=lambda v: v[1], reverse=True)

    for item in edge2betweenness:
...
def brute_force_norm_cut(graph, max_size):
    ...
    ###TODO
    edges= graph.edges()
    comb = list(combinations(edges, max_size))
    for nodes in edges:
      comb.append([nodes])
    allcuts= []
    for pair in comb:
      ...
      for nds in pair:
          removed.append(nds)
          graph_copy.remove_edge(nds[0], nds[1])
      if nx.number_connected_components(graph_copy) == 2:
        components = [i for i in nx.connected_components(graph_copy)]
        S = components[0]
        T = components[1]
        N = norm_cut(S, T, graph)
        allcuts.append((N,removed))
    return allcuts
